...
def play(game_frame=game_frame):
    print('playing')
    ...
    class cell(ctk.CTkFrame):
        def __init__(self, master, seed):
            ...
            def check_cells():
                ...
                self.button.destroy()
                self.active = False
                if self.cell_value >= 1 and self.cell_value <=8:
                    return
                if self.cell_value == 0:
                    for square in game_frame.winfo_children():
                        if square.seed in adjacent_cells_list:
                            square.check_cells()
                if self.cell_value >= 9:
                    lose_game()
                    return
            ...
            self.check_cells = lambda: check_cells()

            self.button = ctk.CTkButton(self.cell_label, width=20, height=20, corner_radius=2, text=None, fg_color = "#525252", command = lambda: check_cells())
...
reset_button = ctk.CTkButton(header_frame, image = happy, command = lambda:reset())
